...
def load_rgb_frames_from_video(video_path, max_frames=64, resize=(256, 256)):
    if not os.path.exists(video_path):
        ...
        return np.array([])  # Return an empty array if the video file does not exist
    ...
    for offset in range(0, min(max_frames * frameskip, int(total_frames - start))):
        ...
        if not success:
            print(f"Failed to read frame at offset {offset} from video: {video_path}")
...
class ASLCitizen(data_utl.Dataset):
    ...
    def pad(self, imgs, total_frames):
        if imgs.shape[0] < total_frames:
            ...
            if num_padding:
                if imgs.size == 0:  # Check if imgs is empty
                    print("No frames available to pad.")
                    return np.zeros((total_frames, 256, 256, 3), dtype=np.float32)  # Return a default padded array
